...
class validators():
    ...
    def __init__(self, samplesheet, regex_patterns=None) -> None:
        ...
        self.bclconvert_samplesheet_body = samplesheet[3]
        self.bclconvert_samplesheet_header = samplesheet[4]
        self.bclconvert_header_count = samplesheet[5]
    ...
    def header(self) -> None:
        ...
        - Last line of header is [TSO550S_Data]
    ...
    def bclconvert_header(self) -> None:
        """
        Validate header against:
        - first line being [BCLConvert_Settings]
        - having Investigator and Experiment names set
        - Valid value for reads
        - Last line of header is [BCLConvert_Data]
        - Column names begin with Sample_ID Sample_Name, all others are
            variable
        """
        bclconvert_header_errors = []
        # regex to match adapter lines as they can be Adapter, Adapter1,
        # AdapterRead1 etc.
        adapter_regex = re.compile("^adapter(read)?[1,2]?", re.IGNORECASE)

        for num, line in enumerate(self.bclconvert_samplesheet_header):
            # check each line of header for specific matches
            if num == 0:
                if not line.startswith('[BCLConvert_Settings]'):
                    # first line should always be [Header]
                    bclconvert_header_errors.append(
                        f'Error in line {num + 1} of header: value should be [BCLConvert_Settings]'
                    )

            if line.startswith('SoftwareVersion'):
                # check software version set
                if not line.split(',')[1]:
                    bclconvert_header_errors.append(
                        f'Error in line {num + 1}: no software version given'
                    )

            if line.startswith('MinimumTrimmedReadLength') or \
                line.startswith('MaskShortReads'):
                # check the next line is a valid integer
                if not line.split(',')[1].isnumeric():
                    bclconvert_header_errors.append((
                        f'Error in value for entry MinimumTrimmedReadLength or \
                            MaskShortReads given on line {num + 1}: '
                        f'{self.bclconvert_samplesheet_header[num]}'
                    ))
        ...
        if bclconvert_header_errors:
            self.errors['bclconvert_header'].extend(bclconvert_header_errors)
        ...
        for count, line in enumerate(f.readlines()):
            ...
            if line.strip().rstrip(',') :
                if 'Sample_ID' not in line and \
                  ...
                  len(bclconvert_header) >= 1 and \
                  len(bclconvert_column_names) == 0 :
                    bclconvert_header.append(line.rstrip())
                elif 'Sample_ID' in line and \
                  len(bclconvert_header) >= 1 :
                    bclconvert_header.append(line.rstrip())
